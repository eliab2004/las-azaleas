...
export async function POST(req:Request){try{mutation(req);const m=await member(req);const input:any=await req.json();role(m,'administrador');const branchId=txt(input.branchId);const b=await db().prepare('SELECT id FROM branches WHERE id=?').bind(branchId).first();if(!b)throw new Error('400|Selecciona una sucursal existente.');
