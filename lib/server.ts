...
export async function member(){const u=await identity();const m=await db().prepare('SELECT * FROM members WHERE email=?').bind(u.email.toLowerCase()).first<any>();if(!m)throw new Error('403|Tu cuenta aún no tiene un rol asignado.');return m;}
...
export function fail(e:unknown){const msg=e instanceof Error?e.message:'';const [code,detail]=msg.split('|');if(['400','401','403','404','409'].includes(code))return Response.json({error:detail},{status:Number(code),headers:{'Cache-Control':'no-store'}});console.error('Request failed',e instanceof Error?e.name:'unknown');return Response.json({error:'No pudimos completar la operación. Intenta nuevamente.'},{status:500});}
