...
export async function POST(req:Request){
 ...
 try{
  ...
  stage='identity';let user:any=null;try{user=await identity(req)}catch{}
  const username=txt(input.username,80).toLowerCase();
  const ownerEmail=(user?.email||username.includes('@')?username:username+'@lasazaleas.local').toLowerCase();
  const ownerName=user?.displayName||username;
  ...
  stage='owner-save';await db().batch([
   ...
   db().prepare(`INSERT INTO members(email,name,role,branch_id,username,password_hash,password_salt,is_owner) VALUES(?,?,?,NULL,?,?,?,1)
    ON CONFLICT(email) DO UPDATE SET name=excluded.name,role='administrador',branch_id=NULL,username=excluded.username,password_hash=excluded.password_hash,password_salt=excluded.password_salt,is_owner=1`).bind(ownerEmail,ownerName,'administrador',username,record.hash,record.salt)
  ]);
  stage='session';const token=await issueSession(ownerEmail);return Response.json({ok:true},{headers:{'Set-Cookie':setSessionCookie(token,req),'Cache-Control':'no-store'}});
