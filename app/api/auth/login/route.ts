...
export async function POST(req:Request){try{
 ...
 const username=txt(input.username,40).toLowerCase(),key=await attemptKey(req,username),now=Date.now(),attempt=await db().prepare('SELECT * FROM login_attempts WHERE id=?').bind(key).first<any>();
 ...
 const user=await db().prepare('SELECT * FROM members WHERE username=?').bind(username).first<any>();const valid=Boolean(user?.password_hash&&user?.password_salt&&await verifyPassword(input.password,user.password_salt,user.password_hash));
